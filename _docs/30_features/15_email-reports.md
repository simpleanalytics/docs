---
title: Email reports
category: features
permalink: /email-reports
last_modified_at: 2026-08-07
---

Want to start your Monday morning by checking how your website is doing? Want to send updates to others? Use our weekly or monthly email reports so you don't have to check the dashboard all the time. Never miss that spike of traffic.

Monthly email reports are enabled by default for new websites. They have also been enabled for eligible members of existing teams. If you don't want to receive these reports, use the one-click link in the first automatically enabled email or turn them off in [your website settings](https://simpleanalytics.com/select-website/settings#email-reports).

In your website settings, you can also add a report or choose between weekly and monthly reports. Look for the section that looks like this:

<img class="border" style="width: 717px; max-width: 100%;" src="../../images/email-report-setting.png" alt="">

The email will look more or less like this:

<img class="border" style="width: 100%;" src="../../images/email-report.png" alt="">

It includes the top 5 referrers and we compare them with the previous period. This gives you an indication of where your website is getting its traffic from, and how those numbers might have changed.

In the example above it shows that we got significantly more traffic from our referral website _(referral.simpleanalytics.com <span style="color: #227522;">+963%</span>)_ than last week. Time for us to find out why. Thanks email report :)

We send this email at 9AM in your local time zone. You can change this time zone per website in your [website settings](https://simpleanalytics.com/select-website/settings#timezone).
