...
We added what was then called a session ID to page views. This makes it possible to link events to page views created during one actual page load. We now call it a page-load ID because, unlike a conventional analytics session, it does not continue across full page loads. The legacy `session_id` field remains available for backward compatibility.
